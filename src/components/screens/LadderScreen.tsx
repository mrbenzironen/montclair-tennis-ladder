...
export function LadderScreen() {
  ...
  const ladderName = user?.profile?.ladder?.name ?? 'Tennis Ladder'

  function getDelta(player: User) {
    // In a real app you'd compare to previous rank stored in DB
    // For now show neutral
    return '—'
  }

  function getDeltaColor(delta: string) {
    if (delta.includes('↑')) return '#5a9e00'
    if (delta.includes('↓')) return '#c0392b'
    return '#cbc8c2'
  }
            ...
            return (
              ...
              >
                ...
                <img
                  ...
                  onError={e => { (e.target as HTMLImageElement).src = `https://ui-avatars.com/api/?name=${encodeURIComponent(player.full_name)}&background=201c1d&color=c4e012&size=72` }}
                />

                {/* Info */}
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 14, fontWeight: 500, color: '#201c1d', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                    {player.full_name}{isMe ? '' : ''}
                  </div>
                  ...
                  {isEligible && !isMe && (
                    <button
                      ...
                      style={{
                        ...
                        padding: '7px 12px', border: '1.5px solid #201c1d',
                        background: 'transparent', color: '#201c1d',
                        borderRadius: 4, cursor: 'pointer', minWidth: 76,
                      }}
                    >
                      Challenge
                    </button>
                  )}
                  <div style={{ fontSize: 16, fontWeight: 700, width: 16, textAlign: 'center', color: getDeltaColor(delta) }}>
                    {delta}
                  </div>
                </div>
              </div>
